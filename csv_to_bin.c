...
{
    REG_HEADER *header = (REG_HEADER *)malloc(sizeof(REG_HEADER));
    if (header == NULL) {
        printf("Falha no processamento do arquivo.\n");
        return NULL;
    }

    // Lê o header do arquivo binário
    fread(&header->status, sizeof(char), 1, bin);
    fread(&header->topo, sizeof(long long int), 1, bin);
    fread(&header->proxByteOffset, sizeof(long long int), 1, bin);
    fread(&header->nroRegArq, sizeof(int), 1, bin);
    fread(&header->nroRegRem, sizeof(int), 1, bin);
    fread(header->descreveIdentificador, sizeof(char), 23, bin);
    fread(header->descreveYear, sizeof(char), 27, bin);
    fread(header->descreveFinancialLoss, sizeof(char), 28, bin);
    fread(&header->codDescreveCountry, sizeof(char), 1, bin);
    fread(header->descreveCountry, sizeof(char), 26, bin);
    fread(&header->codDescreveType, sizeof(char), 1, bin);
    fread(header->descreveType, sizeof(char), 38, bin);
    fread(&header->codDescreveTargetIndustry, sizeof(char), 1, bin);
    fread(header->descreveTargetIndustry, sizeof(char), 38, bin);
    fread(&header->codDescreveDefense, sizeof(char), 1, bin);
    fread(header->descreveDefense, sizeof(char), 67, bin);
