...
void match_registro(FILE *bin, int num_campos, char **nome_campo, char **valor_campo)
{
    ...
    // Inicializa ponteiro para a struct de registro de dados e as variáveis de controle
    ...
    while (ftell(bin) != -1 && reg_dados != NULL) {
        ...
        if (match) {
            ...
            // Libera a memória apenas se não imprimiu (pois imprime_registro_bin já libera)
