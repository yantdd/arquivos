...
REG_DADOS *aplica_atualizacao(REG_DADOS *reg_original, int num_campos_atualizacao, char **nomes_campos_atualizacao, char **valores_campos_atualizacao) {
    ...
    if(strcmp(reg_original->country, "NADA CONSTA") == 0) {
        strcpy(reg_atualizado->country, "NULO");
    }
    else {
        strcpy(reg_atualizado->country, reg_original->country);
    }
    if(strcmp(reg_original->attackType, "NADA CONSTA") == 0) {
        strcpy(reg_atualizado->attackType, "NULO");
    }
    else {
        strcpy(reg_atualizado->attackType, reg_original->attackType);
    }
    if(strcmp(reg_original->targetIndustry, "NADA CONSTA") == 0) {
        strcpy(reg_atualizado->targetIndustry, "NULO");
    }
    else {
        strcpy(reg_atualizado->targetIndustry, reg_original->targetIndustry);
    }
    if(strcmp(reg_original->defenseMechanism, "NADA CONSTA") == 0) {
        strcpy(reg_atualizado->defenseMechanism, "NULO");
    }
    else {
...
/*
 * Calcula o tamanho físico real ocupado por um registro no arquivo
 * Parâmetros:
 *  bin - ponteiro para o arquivo binário
 *  posicao_registro - posição do início do registro
 * Retorno:
 *  tamanho físico real ocupado no arquivo
 */
int calcula_tamanho_fisico_registro(FILE *bin, long long posicao_registro) {
    long long posicao_atual = ftell(bin);
    
    // Vai para a posição do registro e lê o tamanhoRegistro
    fseek(bin, posicao_registro + sizeof(char), SEEK_SET); // Pula o campo removido
    int tamanho_logico;
    fread(&tamanho_logico, sizeof(int), 1, bin);
    
    // O tamanho físico = campos fixos (char + int + long long) + tamanho lógico
    int tamanho_fisico = sizeof(char) + sizeof(int) + tamanho_logico;
    
    // Volta para posição original
    fseek(bin, posicao_atual, SEEK_SET);
    
    return tamanho_fisico;
}
 ...
 *  tamanho_fisico_original - tamanho físico original ocupado no arquivo
 */
void escreve_registro_atualizado_no_local(FILE *bin, REG_DADOS *reg_atualizado, int tamanho_fisico_original) {
    // Marca posição inicial para calcular bytes escritos
    long posicao_inicial = ftell(bin);
    ...
    // Calcula bytes realmente escritos
    long posicao_atual = ftell(bin);
    int bytes_escritos = posicao_atual - posicao_inicial;
    int lixo_restante = tamanho_fisico_original - bytes_escritos;
...
void match_registro_atualiza(FILE *bin, REG_HEADER *header, int num_campos_busca, char **nome_campo_busca, char **valor_campo_busca, int num_campos_atualizacao, char **nomes_campos_atualizacao, char **valores_campos_atualizacao) {
    ...
    while (true) {
        ...
        // Verifica se o registro faz match com os critérios de busca
        bool match = true;
        ...
        // Se encontrou um match, armazena a posição
        if (match) {
            ...
            posicoes_match[count_matches++] = posicao_registro;
        }
    ...
    // SEGUNDA PASSADA: Aplicar atualizações em ordem reversa (do fim para o início)
    for (int i = count_matches - 1; i >= 0; i--) {
        fseek(bin, posicoes_match[i], SEEK_SET);
        REG_DADOS *reg_dados = get_registro(bin);
        
        if (reg_dados != NULL && reg_dados->removido == '0') {
            // Aplica as atualizações
            ...
            if (reg_atualizado != NULL) {
                ...
                if (tamanho_atualizado <= tamanho_original) {
                    ...
                    int tamanho_fisico_original = calcula_tamanho_fisico_registro(bin, posicoes_match[i]);
                    escreve_registro_atualizado_no_local(bin, reg_atualizado, tamanho_fisico_original);
                } else {
                    ...
                    // Correção: o resultado líquido deveria ser neutro
                    header->nroRegArq--;
                    header->nroRegRem++;
                }
